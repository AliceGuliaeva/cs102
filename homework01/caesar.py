...
def encrypt_caesar(plaintext: str, shift: int = 3) -> str:
    ...
    for i in range(len(plaintext)):
        if (ord(plaintext[i])<=ord('z') and ord(plaintext[i])>=ord('a')):
            if ord(plaintext[i])>=(ord('z')-shift+1):
                ciphertext+=chr((ord(plaintext[i])+shift-1)%ord('z')+ord('a'))
            else:
...
def decrypt_caesar(ciphertext: str, shift: int = 3) -> str:
    ...
    for i in range(len(ciphertext)):
        if (ord(ciphertext[i])<=ord('z') and ord(ciphertext[i])>=ord('a')):
            if ord(ciphertext[i])<=(ord('a')+shift-1):
                plaintext+=chr(ord(ciphertext[i])-ord('a')-shift+1+ord('z'))
            else:
                plaintext+=chr(ord(ciphertext[i])-shift)
        elif (ord(ciphertext[i])<=ord('Z') and ord(ciphertext[i])>=ord('A')):
            if ord(ciphertext[i])<=(ord('A')+shift-1):
                plaintext+=chr(-ord('A')-shift+1+ord(ciphertext[i])+ord('Z'))
            else:
                plaintext+=chr(ord(ciphertext[i])-shift)
        else:
